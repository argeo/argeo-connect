...
import org.argeo.people.ImportService;
...
public class ImportServiceImpl implements ImportService, PeopleNames {
	...
	// Filtered properties
	private final List<String> TECHNICAL_PROPERTIES = asList("jcr:uuid", "jcr:baseVersion", "jcr:isCheckedOut",
			"jcr:predecessors", "jcr:frozenUuid", "jcr:versionHistory", "jcr:frozenPrimaryType", "jcr:primaryType",
			"jcr:mixinTypes", "jcr:created", "jcr:createdBy", "jcr:lastModified", "jcr:lastModifiedBy");
	...
	public void mergeNodes(Node masterNode, Node slaveNode) throws RepositoryException {
		...
		nodes: while (nit.hasNext()) {
			...
			if (masterNode.hasNode(currNode.getName()))
				...
				masterCurrChild = masterNode.addNode(currNode.getName(), currNode.getPrimaryNodeType().getName());
	...
	protected void mergeJcrReferences(Node masterNode, Node slaveNode) throws RepositoryException {
		...
		while (pit.hasNext()) {
			...
			if (ref.isMultiple()) {
				ConnectJcrUtils.removeRefFromMultiValuedProp(referencing, ref.getName(), slaveNode.getIdentifier());
				ConnectJcrUtils.addRefToMultiValuedProp(referencing, ref.getName(), masterNode);
			} else
	...
	private void mergeMultipleProperty(Node masterNode, Property masterProp, Property slaveProp)
		...
		if (masterProp == null) {
			...
			if (vals[0].getType() == PropertyType.STRING) {
				...
				ValueFactory vFactory = masterNode.getSession().getValueFactory();
				int size = res.size();
				Value[] values = new Value[size];
				int i = 0;
				for (String id : res) {
					Value val = vFactory.createValue(id, PropertyType.REFERENCE);
					values[i++] = val;
				}
				masterNode.setProperty(slaveProp.getName(), values);
			} else {
				throw new PeopleException(
						"Unsupported multiple property type on property " + masterProp + "for node " + masterNode);
			}
		}
	}
}
