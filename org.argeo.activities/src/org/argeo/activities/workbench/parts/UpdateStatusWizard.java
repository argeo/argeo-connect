 ...
 */
public class UpdateStatusWizard extends Wizard {
	...
	public void addPages() {
		try {
			...
			throw new ActivitiesException("Cannot add page to wizard", e);
		}
	}
	...
	public boolean performFinish() {
		...
		new UpdateStatusesJob(activitiesService, nodes, taskId, chosenStatus).schedule();
		return true;
	}
	...
	private class UpdateStatusesJob extends PrivilegedJob {
		...
		public UpdateStatusesJob(ActivitiesService activityService, Node[] toUpdateNodes, String taskTypeId,
			...
			try {
				...
				throw new ActivitiesException("Unable to initialise " + "status batch update ", e);
			}
		}
		...
		protected IStatus doRun(IProgressMonitor progressMonitor) {
			...
			try {
				...
				return new Status(IStatus.ERROR, ActivitiesUiPlugin.PLUGIN_ID,
						"Unable to perform batch update to status " + chosenStatus + " on " + taskTypeId
								+ " for node list ",
						e);
			} finally {
