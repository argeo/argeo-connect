 ...
 * FIXME clean this
 * 
 * @deprecated introduced to workaround the deadlock exception with the user
 *             admin wrapper.
 * 
 * */
...
public class UserAdminUtils {
	...
	public static String getUserUid(String dn) {
		...
		if (last.getType().toLowerCase().equals(LdapAttrs.uid.name())
			...
			throw new PeopleException("Cannot retrieve user uid, "
					+ "non valid dn: " + dn);
	}
	...
	public static LdapName getLdapName(String dn) {
		try {
			...
			throw new PeopleException("Cannot parse LDAP name " + dn, e);
		}
	}
}