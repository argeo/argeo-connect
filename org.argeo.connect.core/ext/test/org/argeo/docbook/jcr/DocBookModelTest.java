...
import org.argeo.docbook.jcr.DocBookModel;
...
public class DocBookModelTest extends AbstractJackrabbitTestCase {
	...
	protected void importXml(String res, Boolean mini) throws Exception {
		...
		{// cnd
			...
			if (mini) {
				InputStreamReader reader = new InputStreamReader(
						getClass().getResourceAsStream("/org/argeo/docbook/jcr/docbook.cnd"));
				CndImporter.registerNodeTypes(reader, session());
				reader.close();
			} else {
				InputStreamReader reader = new InputStreamReader(
						getClass().getResourceAsStream("/org/argeo/docbook/jcr/docbook-full.cnd"));
				CndImporter.registerNodeTypes(reader, session());
				reader.close();
			}
		...
		try (InputStream in = new ByteArrayInputStream(bytes)) {
			...
			long duration = System.currentTimeMillis() - begin;
			if (log.isDebugEnabled())
				log.debug("Imported " + res + " " + (bytes.length / 1024l) + " kB in " + duration + " ms ("
		...
		try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
			...
			try (InputStream in = new ByteArrayInputStream(bytes)) {
				...
				long duration = System.currentTimeMillis() - begin;
				if (log.isDebugEnabled())
					log.debug("Re-imported " + res + " " + (bytes.length / 1024l) + " kB in " + duration + " ms ("
							+ (bytes.length / duration) + " B/ms)");
			}
		}
		saveSession();
