...
public class JxlUtils {
	...
	public static String getCompulsoryStringValue(Sheet sheet, int x, int y) {
		...
		if (EclipseUiUtils.isEmpty(stringValue))
			throw new ConnectException("No name defined at [" + x + "," + y
					+ "], cannot parse indicator file");
		return stringValue;
	}
	...
	public static Double getNumberValue(Sheet sheet, int x, int y) {
		...
		if (type == CellType.NUMBER)
			...
			throw new ConnectException("Not a number at [" + x + "," + y + "]: "
					+ type.toString());
	}

	public static Sheet getOnlySheet(InputStream in, String encoding)
			throws IOException {
	...
	public static Sheet getSheet(InputStream in, String encoding, int index)
			throws IOException {
		Workbook wkb = toWorkbook(in, encoding);
		return wkb.getSheet(index);
	}

	public static Workbook toWorkbook(InputStream in, String encoding)
			throws IOException {
		try {
