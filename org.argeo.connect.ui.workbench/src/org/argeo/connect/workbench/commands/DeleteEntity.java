...
public class DeleteEntity extends AbstractHandler {
	...
	public final static String ID = ConnectUiPlugin.PLUGIN_ID + ".deleteEntity";
	...
	public Object execute(final ExecutionEvent event) throws ExecutionException {
		...
		try {
			...
			boolean result = MessageDialog.openConfirm(activeShell, "Confirm Deletion", msg);
			...
			if (!ConnectJcrUtils.checkCOStatusBeforeUpdate(toRemoveNode))
				log.warn("To remove node " + toRemoveNode + " was checked in when we wanted to remove it");

			parentVersionableNode = ConnectJcrUtils.getParentVersionableNode(toRemoveNode);

			if (parentVersionableNode != null && toRemoveNode.getPath().equals(parentVersionableNode.getPath()))
				parentVersionableNode = null;

			if (parentVersionableNode != null) {
				if (!ConnectJcrUtils.checkCOStatusBeforeUpdate(parentVersionableNode))
			...
			if (iep != null && iep.getEditorInput().getName().equals(toRemoveJcrId))
				iwp.closeEditor(iep, false);

		} catch (ReferentialIntegrityException e) {
			MessageDialog.openError(activeShell, "Delete impossible",
			...
			throw new ConnectException("Unable to delete node " + toRemoveNode, e);
		} finally {
	...
	private boolean canDelete(Node node, Shell activeShell) throws RepositoryException {
		List<Node> related = new ArrayList<Node>();

		// TODO peform beter check before trying to remove
		...
		if (related.isEmpty())
			...
			String msg = "Current object is referenced by following entities: " + getRelatedAsString(related) + "\n";
	...
	private String getRelatedAsString(List<Node> related) throws RepositoryException {

		return "";

		// Was used to check if people deletion is valid.
		// if (related.isEmpty())
		// return "";
		...
		// builder.append(node.getProperty(PeopleNames.PEOPLE_ROLE).getString());
		// Node parent = node.getParent().getParent();
		// if (parent.hasProperty(Property.JCR_TITLE))
		// builder.append("
		// (").append(parent.getProperty(Property.JCR_TITLE).getString()).append(")");
